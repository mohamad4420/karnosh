    ...
    return (
        ...
        <div className="serverRight">
            {
                servers.map((data,i)=>{
                    if(i%2!=0)
                    return(
                        <Button variant={style[i]} onClick={()=>{setUrl(data)}} color='secondary'>{data.replace('https://','').replace('www.','').split('.')[0]}</Button>
                    )
                })
            }
           </div>
        <div className="serverLeft">
        {
                servers.map((data,i)=>{
                    if(i%2==0)
                    return(
                        <Button variant={style[i]}s onClick={()=>{setUrl(data)}} color='primary'>{data.replace('https://','').replace('www.','').split('.')[0]}</Button>
                    )
                })
            }
        </div>
