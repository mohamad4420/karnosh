    ...
    return (
        ...
        <div className="serverRight">
            {
                servers.map((data,i)=>{
                    if(i%2==0)
                    return(
                        <Button variant="outlined" color='secondary'>{data.replace('https://','').replace('www.','').split('.')[0]}</Button>
                    )
                })
            }
           </div>
        <div className="serverLeft">
        {
                servers.map((data,i)=>{
                    if(i%2!=0)
                    return(
                        <Button variant="outlined" color='primary'>{{data.replace('https://','').replace('www.','').split('.')[0]}}</Button>
                    )
                })
            }
        </div>
