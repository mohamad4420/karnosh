...
keyPress=(event)=>{
  ...
  axios.post('https://karnoshapi.herokuapp.com/dataVideo/search',{"name":event.target.value})
  .then((res)=>{
  ...
  var th = this;
   ...
   window.addEventListener("scroll",async function(){
    ...
    if(myhight>hight/8 && window.innerWidth>500){
      th.setState({style:"black"})
    }
    else if(myhight>hight/8 ){
      th.setState({MobileStyle:"black"})
    }
    else{
...
this.state.data.map((data,i) =>
  <>
          <Link href={'/Watch/'+encodeURI(data.name.replace(/ /g,'-'))} key={Math.random() * 1000000000}>
             <a>
             <div className="prodectMain" onClick={()=>{this.setState({openSearch:false})}}>
               ...
               <img src={data.poster.search('amazon')==-1?data.poster:data.poster.replace('.jpg','._V1_UX380_CR0,0,380,542_AL_.jpg')}  quality={10} layout='fill' />
             </div>
             </a>
          </Link>
